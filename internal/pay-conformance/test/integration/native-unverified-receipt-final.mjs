...
import { currentUnverifiedReceiptCases, resolveFinalUnverifiedReceiptProfile } from "../../src/ipc.mjs";
...
const row = `${input.fixture}-unverified-receipt`;
const profile = resolveFinalUnverifiedReceiptProfile(input.fixture, row, input.stage);
const caseIds = currentUnverifiedReceiptCases;
...
test(input.fixture + " final unverified receipt controls", async t => {
  ...
  const run = await runProcess({ command: [process.execPath, join(root, contract.driver), inputPath, contract.id, directory, "unverified-receipt-controls"], cwd: directory, env, expectedVersions: contract.expectedVersions, timeoutMs: 60000 });
  await writeFile(join(directory, "unverified-receipt.process.json"), JSON.stringify(run, null, 2) + "\n", { flag: "wx", mode: 0o600 });
  ...
  assert.deepEqual(observed.catalog, currentUnverifiedReceiptCases);
  assert.deepEqual(observed.receiptContract, { protocol: profile.protocol, owner: profile.owner, version: profile.version, failurePending: true, proofRpcCalls: 4, proofBeforeClear: true, auditedVerifierCases: 2 });

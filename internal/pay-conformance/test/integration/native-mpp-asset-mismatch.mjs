...
import { currentMppAssetMismatchCases, resolveFinalMppAssetMismatchProfile } from "../../src/ipc.mjs";
...
const profile = resolveFinalMppAssetMismatchProfile(input.fixture, row, input.stage);
...
test(`${input.fixture} asset mismatch is complete before final-7b admission`, async () => {
  ...
  const run = await runProcess({ command: [process.execPath, join(root, contract.driver), inputPath, contract.id, directory, "mpp-asset-mismatch-controls"], cwd: directory, env, expectedVersions: contract.expectedVersions, timeoutMs: 60000, maxOutputBytes: 4194304 });
  await writeFile(join(directory, "mpp-asset-mismatch.process.json"), JSON.stringify(run, null, 2) + "\n", { flag: "wx", mode: 0o600 });
  ...
  assert.equal(observed.artifactSha256, input.consumer.artifactSha256); assert.deepEqual(observed.catalog, currentMppAssetMismatchCases);
  assert.deepEqual(observed.assetContract, { owner: profile.owner, version: profile.version, codecOwner: profile.codecOwner, offerField: "request.currency", offerDecimalsField: "request.methodDetails.decimals", wireField: "challenge.request.currency" });
  const expected = Object.entries(currentMppAssetMismatchCases).flatMap(([path, ids]) => ids.flatMap(caseId => ["import", "require"].map(condition => [path, caseId, condition])));
  ...
  for (const subcase of observed.subcases) {
    ...
    if (subcase.path === "offer") {
      ...
      assert.deepEqual([changed.field, changed.codecOwner, changed.decoder, changed.encoder], [subcase.caseId === "wrong-decimals" ? "request.methodDetails.decimals" : "request.currency", profile.codecOwner, "Challenge.fromResponse", "Challenge.serialize"]);
      ...
      assert.equal(changed.decodedDecimals, subcase.caseId === "wrong-decimals" ? 18 : 6);
      assert.equal(calibration.decodedDecimals, 6);
      for (const change of [changed, calibration]) {
        for (const key of ["unchangedBeforeSha256", "unchangedAfterSha256", "envelopeBeforeSha256", "envelopeAfterSha256"]) assert.match(change[key], /^[a-f0-9]{64}$/);
        assert.equal(change.unchangedBeforeSha256, change.unchangedAfterSha256); assert.equal(change.envelopeBeforeSha256, change.envelopeAfterSha256);
      }
      for (const change of [changed, calibration]) assert.match(change.decodedAssetSha256, /^[a-f0-9]{64}$/);
    } else {
      const [negative, positive] = subcase.checkpoints;
      assert.deepEqual([negative.buyer.counters.sign, negative.buyer.counters.signedSend, negative.buyer.status, negative.buyer.classification], [1, 1, 402, "invalid-challenge"]);
      ...
      assert.deepEqual([negative.buyer.wire.field, negative.buyer.wire.codecOwner, negative.buyer.wire.decoder, negative.buyer.wire.encoder], ["challenge.request.currency", profile.codecOwner, "Credential.deserialize", "Credential.serialize"]);
      assert.notEqual(negative.buyer.wire.originalSha256, negative.buyer.wire.transmittedSha256);
      assert.equal(negative.buyer.wire.challengeRemainderBeforeSha256, negative.buyer.wire.challengeRemainderAfterSha256); assert.equal(negative.buyer.wire.payloadBeforeSha256, negative.buyer.wire.payloadAfterSha256);

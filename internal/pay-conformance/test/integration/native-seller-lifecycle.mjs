...
const cases = {
  "handler-failure": ["handler-throws", "handler-500", "handler-400", "handler-404", "handler-302", "handler-200", "fulfillment-failed-after-handler-failure"],
  "fulfillment-failure": ["fulfillment-http-503", "fulfillment-disconnect", "fulfillment-timeout", "fulfillment-unexpected-2xx"],
};
const familyFilter = process.argv[3];
assert.ok(familyFilter === undefined || Object.hasOwn(cases, familyFilter));
for (const [family, caseIds] of Object.entries(cases).filter(([family]) => !familyFilter || familyFilter === family)) test(input.fixture + "-" + family + " partial native buyer to Pay seller lifecycle", async t => {
  ...
  const run = await runProcess({ command: [process.execPath, join(root, contract.driver), inputPath, contract.id, directory, "seller-controls"], cwd: directory, env, expectedVersions: contract.expectedVersions, timeoutMs: 60000 });
  await writeFile(join(directory, "seller.process.json"), JSON.stringify(run, null, 2) + "\n", { flag: "wx", mode: 0o600 });
  ...
  const observed = JSON.parse(await readFile(join(directory, "seller-observations.json")));
  assert.equal(observed.scope, "seller-controls-slice"); assert.equal(observed.coverage, "partial"); assert.equal(observed.aggregateStatus, "BLOCKED");

...
import { currentMppAmountMismatchCases, resolveFinalMppAmountMismatchProfile } from "../../src/ipc.mjs";
...
const row = `${input.fixture}-amount-mismatch`;
const profile = resolveFinalMppAmountMismatchProfile(input.fixture, row, input.stage);

test(`${input.fixture} amount mismatch is complete before final-7b admission`, async () => {
  ...
  const run = await runProcess({ command: [process.execPath, join(root, contract.driver), inputPath, contract.id, directory, "mpp-amount-mismatch-controls"], cwd: directory, env, expectedVersions: contract.expectedVersions, timeoutMs: 60000, maxOutputBytes: 4194304 });
  await writeFile(join(directory, "mpp-amount-mismatch.process.json"), JSON.stringify(run, null, 2) + "\n", { flag: "wx", mode: 0o600 });
  ...
  assert.equal(observed.artifactSha256, input.consumer.artifactSha256); assert.deepEqual(observed.catalog, currentMppAmountMismatchCases);
  assert.deepEqual(observed.amountContract, { owner: profile.owner, version: profile.version, codecOwner: profile.codecOwner, offerField: "request.amount", wireField: "payload.value" });
  const expected = Object.entries(currentMppAmountMismatchCases).flatMap(([path, ids]) => ids.flatMap(caseId => ["import", "require"].map(condition => [path, caseId, condition])));
  assert.deepEqual(observed.subcases.map(value => [value.path, value.caseId, value.condition]), expected); assert.equal(observed.subcases.length, 10);
  for (const subcase of observed.subcases) {
    ...
    if (subcase.path === "offer") {
      ...
      assert.deepEqual([changed.field, changed.codecOwner, changed.decoder, changed.encoder], ["request.amount", profile.codecOwner, "Challenge.fromResponse", "Challenge.serialize"]);
      ...
      for (const change of [changed, calibration]) {
        for (const key of ["decodedAmountSha256", "unchangedBeforeSha256", "unchangedAfterSha256", "envelopeBeforeSha256", "envelopeAfterSha256"]) assert.match(change[key], /^[a-f0-9]{64}$/);
      ...
      assert.deepEqual([negative.buyer.wire.field, negative.buyer.wire.codecOwner, negative.buyer.wire.decoder, negative.buyer.wire.encoder], ["payload.value", profile.codecOwner, "Credential.deserialize", "Credential.serialize"]);
      assert.notEqual(negative.buyer.wire.originalSha256, negative.buyer.wire.transmittedSha256);
      assert.equal(negative.buyer.wire.challengeBeforeSha256, negative.buyer.wire.challengeAfterSha256); assert.equal(negative.buyer.wire.payloadRemainderBeforeSha256, negative.buyer.wire.payloadRemainderAfterSha256);

...
import { currentX402AmountMismatchCases, resolveFinalX402AmountMismatchProfile } from "../../src/ipc.mjs";
...
const profile = resolveFinalX402AmountMismatchProfile(input.fixture, row, input.stage);
...
test(`${input.fixture} amount mismatch is complete before final-7b admission`, async () => {
  ...
  const run = await runProcess({ command: [process.execPath, join(root, contract.driver), inputPath, contract.id, directory, "amount-mismatch-controls"], cwd: directory, env, expectedVersions: contract.expectedVersions, timeoutMs: 60000, maxOutputBytes: 4194304 });
  await writeFile(join(directory, "amount-mismatch.process.json"), JSON.stringify(run, null, 2) + "\n", { flag: "wx", mode: 0o600 });
  ...
  assert.equal(observed.artifactSha256, input.consumer.artifactSha256); assert.deepEqual(observed.catalog, currentX402AmountMismatchCases);
  assert.deepEqual(observed.amountContract, { owner: profile.owner, version: profile.version, codecOwner: profile.codecOwner, offerField: "accepts.amount", wireField: "payload.authorization.value" });
  const expected = Object.entries(currentX402AmountMismatchCases).flatMap(([path, ids]) => ids.flatMap(caseId => ["import", "require"].map(condition => [path, caseId, condition])));
  ...
  for (const subcase of observed.subcases) {
    ...
    if (subcase.path === "offer") {
      ...
      assert.equal(subcase.merchant.offerChanges[0].field, "accepts.amount"); assert.equal(subcase.merchant.offerChanges[1].field, "none");
      ...
      assert.equal(negative.buyer.wire.field, "payload.authorization.value"); assert.notEqual(negative.buyer.wire.originalSha256, negative.buyer.wire.transmittedSha256);

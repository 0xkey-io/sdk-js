...
import { currentX402PayeeMismatchCases, resolveFinalX402PayeeMismatchProfile } from "../../src/ipc.mjs";
...
const profile = resolveFinalX402PayeeMismatchProfile(input.fixture, row, input.stage);

test(`x402 ${profile.version} payee mismatch is complete before final-7b admission`, async () => {
  ...
  const run = await runProcess({ command: [process.execPath, join(root, contract.driver), inputPath, contract.id, directory, "payee-mismatch-controls"], cwd: directory, env, expectedVersions: contract.expectedVersions, timeoutMs: 60000, maxOutputBytes: 4194304 });
  await writeFile(join(directory, "payee-mismatch.process.json"), JSON.stringify(run, null, 2) + "\n", { flag: "wx", mode: 0o600 });
  ...
  assert.equal(observed.artifactSha256, input.consumer.artifactSha256); assert.deepEqual(observed.catalog, currentX402PayeeMismatchCases);
  assert.deepEqual(observed.payeeContract, { owner: profile.owner, version: profile.version, codecOwner: profile.codecOwner, offerField: "accepts.payTo", wireField: "payload.authorization.to", offerBoundary: "invalid-address-syntax-only; no independent payee allowlist" });
  const expected = Object.entries(currentX402PayeeMismatchCases).flatMap(([path, ids]) => ids.flatMap(caseId => ["import", "require"].map(condition => [path, caseId, condition])));
  ...
  for (const subcase of observed.subcases) {
    ...
    if (subcase.path === "offer") {
      ...
      assert.equal(subcase.merchant.offerChanges[0].field, "accepts.payTo"); assert.equal(subcase.merchant.offerChanges[1].field, "none");
      for (const change of subcase.merchant.offerChanges) { assert.match(change.unchangedBeforeSha256, /^[a-f0-9]{64}$/); assert.equal(change.unchangedBeforeSha256, change.unchangedAfterSha256); }
      ...
      assert.deepEqual([negative.buyer.counters.sign, negative.buyer.counters.signedSend, negative.buyer.status, negative.buyer.classification], [1, 1, 400, "PAYMENT_CREDENTIAL_INVALID"]);
      ...
      assert.equal(negative.buyer.wire.field, "payload.authorization.to"); assert.notEqual(negative.buyer.wire.originalSha256, negative.buyer.wire.transmittedSha256);

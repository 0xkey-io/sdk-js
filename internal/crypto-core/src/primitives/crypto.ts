...
export const hpkeEncrypt = ({
  ...
  try {
    ...
    const scheduleInfo = getHpkeScheduleInfo(hpkeInfo);
...
/**
 * Encrypt a message to a quorum key. Algorithm originally implemented in qos here: https://github.com/0xkey-io/qos/blob/ae01904c756107f850aea42000137ef124df3fe4/src/qos_p256/src/encrypt.rs#L123
 ...
 * This encryption function is meant to be used with this decryption function in QOS: https://github.com/0xkey-io/qos/blob/ae01904c756107f850aea42000137ef124df3fe4/src/qos_p256/src/encrypt.rs#L52
 *
...
export const quorumKeyEncrypt = async (
  ...
  // create the additional data in the form of sender_public||sender_public_len||receiver_public||receiver_public_len taken from QOS here: https://github.com/0xkey-io/qos/blob/ae01904c756107f850aea42000137ef124df3fe4/src/qos_p256/src/encrypt.rs#L298
  ...
  const encryptedMessageBuf = await crypto.subtle.encrypt(
    alg,
    cipher,
...
const labeledExtract = (
  ...
  const labeledIkm = buildLabeledIkm(textToBytes(label), ikm, suiteId);
  return new Uint8Array(hkdf.extract(sha256, labeledIkm, new Uint8Array([])));
};

/**
 * Resolve the HPKE key-schedule info used during AES key / nonce derivation.
 *
 * - `undefined` or `"turnkey_hpke"`: legacy compatibility branch. It uses fixed
 ...
 *   can still decrypt older ciphertext during migration.
 ...
 *   the HPKE `info` parameter. This is the branch that the 0xkey enclave and
 *   all native SDK helpers in `packages/crypto/src/0xkey.ts` use.
 *
 * Callers in 0xkey native flows MUST pass `hpkeInfo: "0xkey_hpke"` explicitly;
 * relying on the default is reserved for legacy interop only. Once no live
 * legacy ciphertext needs to be read, this branch can be removed in a breaking
 * release.
 */
const getHpkeScheduleInfo = (
  hpkeInfo?: string,
): { aesKeyInfo: Uint8Array; ivInfo: Uint8Array } => {
  if (!hpkeInfo || hpkeInfo === "turnkey_hpke") {
    return { aesKeyInfo: AES_KEY_INFO, ivInfo: IV_INFO };
  }
  ...
  const infoHash = labeledExtract(
    "info_hash",
    textToBytes(hpkeInfo),
    SUITE_ID_2,
  );
  const keyScheduleContext = new Uint8Array(
    1 + pskIdHash.length + infoHash.length,
  );
  keyScheduleContext.set([0], 0);
...
async function createQuorumKeyEncryptCipher(
  ...
  // create the preImage as defined in qos here: https://github.com/0xkey-io/qos/blob/ae01904c756107f850aea42000137ef124df3fe4/src/qos_p256/src/encrypt.rs#L273-L282
  ...
  const mac = new Uint8Array(
    await crypto.subtle.sign(
      "HMAC",
      hmacKey,
      toArrayBuffer(QOS_ENCRYPTION_HMAC_MESSAGE),
    ),
  );
...
/// This is taken from QOS here: https://github.com/0xkey-io/qos/blob/ae01904c756107f850aea42000137ef124df3fe4/src/qos_p256/src/encrypt.rs#L298
///

...
export const DEFAULT_ZEROXKEY_CLOUD_STORAGE_KEY = "ZEROXKEY_API_KEY";

/**
 * Stamper to use within a `ZeroXKeyClient`
 */
export class TelegramCloudStorageStamper {
  // This stamper uses a typical @0xkey-io/api-key-stamper under the hood and abstracts away the storage of the actual API keys
  ...
  async setSigningKey(config?: TTelegramCloudStorageStamperConfig) {
    if (!config) {
      ...
      let { apiPublicKey, apiPrivateKey } =
        (await this.getAPIKey(DEFAULT_ZEROXKEY_CLOUD_STORAGE_KEY)) ?? {};
      ...
      await this.insertAPIKey(
        ...
        DEFAULT_ZEROXKEY_CLOUD_STORAGE_KEY,
      );
  ...
  async insertAPIKey(
    apiPublicKey: string,
    apiPrivateKey: string,
    key: string = DEFAULT_ZEROXKEY_CLOUD_STORAGE_KEY,
  ) {

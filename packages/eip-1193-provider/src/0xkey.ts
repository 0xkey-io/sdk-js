import { ZeroXKeyClient, isHttpClient } from "@0xkey-io/http";
import type { ZeroXKeyBrowserClient } from "@0xkey-io/sdk-browser";
import type {
  TSignRawPayloadResponse,
  TSignTransactionResponse,
} from "@0xkey-io/http/src/__generated__/services/coordinator/public/v1/public_api.fetcher";
import type { definitions } from "@0xkey-io/http/src/__generated__/services/coordinator/public/v1/public_api.types";

import { signatureToHex } from "viem";
import { pad } from "viem/utils";

import { ZEROXKEY_ERROR_CODE } from "./constants";
...
export async function signMessage({
  ...
  client: ZeroXKeyClient | ZeroXKeyBrowserClient | ZeroXKeySDKClientBase;
  message: `0x${string}`;
 ...
 */
export const zeroXKeyIsDisconnected = (error: { code: number }) => {
  const { WALLET_NOT_FOUND, ORG_NOT_FOUND } = ZEROXKEY_ERROR_CODE;

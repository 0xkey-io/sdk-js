...
import type {
  ...
  ZeroXKeyEIP1193ProviderOptions,
} from "./types";
import { signMessage, signTransaction, zeroXKeyIsDisconnected } from "./0xkey";

import { ChainIdMismatchError, UnrecognizedChainError } from "./errors";
import { VERSION } from "./version";

export { ZeroXKeyEIP1193Provider };

export const createEIP1193Provider = async (
  options: ZeroXKeyEIP1193ProviderOptions,
) => {
  const { zeroXKeyClient, organizationId, walletId, chains } = options;
  ...
  const request: ZeroXKeyEIP1193Provider["request"] = async ({
    method,
    params,
  }) => {
    try {
      switch (method) {
        ...
        case "eth_requestAccounts": {
          ...
          const walletAccounts = await zeroXKeyClient.getWalletAccounts({
            organizationId,
            walletId,
          });
      ...
      if (
        ...
        (error instanceof ZeroXKeyRequestError && zeroXKeyIsDisconnected(error))
      ) {

// Clone only spacing rules and scope them under `.zeroxkey-modal` with !important.
...
// but only scoping it to `.zeroxkey-modal`.
//
// Options:
//   prefix: string  — default ":where(.zeroxkey-modal)"
...
const plugin = (opts = {}) => {
  const prefix = opts.prefix || ":where(.zeroxkey-modal)";
  const strict = opts.strict !== false; // default true

  return {
    postcssPlugin: "postcss-zeroxkey-modal-spacing-important",
    Once(root) {
      root.walkRules((rule) => {
        // Avoid re-processing already-scoped rules
        if (rule.selector.includes(".zeroxkey-modal")) return;
        if (rule.selector.includes(":where(.zeroxkey-modal)")) return;

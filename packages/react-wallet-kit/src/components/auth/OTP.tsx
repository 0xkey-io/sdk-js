...
import { OtpType, ZeroXKeyError, ZeroXKeyErrorCodes } from "@0xkey-io/core";
...
interface OtpVerificationProps {
  contact: string;
  otpId: string;
  otpEncryptionTargetBundle: string; // Required: enclave-signed target bundle for HPKE encryption of the OTP attempt
  otpType: OtpType;
...
export function OtpVerification(props: OtpVerificationProps) {
  ...
  // server). We track it locally so that the next completeOtp call uses the
  // bundle that matches the otpId we will actually submit.
  ...
  const handleContinue = async (otpCode: string) => {
    try {
      ...
      const niceError =
        (error as ZeroXKeyError).code === ZeroXKeyErrorCodes.INVALID_OTP_CODE
          ? "Invalid OTP code"

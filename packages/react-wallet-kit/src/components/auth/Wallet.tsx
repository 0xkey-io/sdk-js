...
import type { WalletProvider } from "@0xkey-io/core";
import { QRCodeSVG as QRCode } from "qrcode.react";
import { SuccessPage } from "../design/Success";
import { isEthereumProvider, isSolanaProvider } from "@0xkey-io/core";
...
export function WalletAuthButton(props: WalletAuthButtonProps) {
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      >
        <FontAwesomeIcon icon={faWallet} className="shrink-0" />
...
export function ExternalWalletChainSelector(
  ...
  const { walletProviders } = useZeroXKey();
...
export function ExternalWalletSelector(props: ExternalWalletSelectorProps) {
  ...
  return Object.keys(grouped).length === 0 ? (
    ...
    <div
      className={clsx(
        "w-72 min-h-42 max-h-64 mt-12 overflow-y-auto zeroxkey-scrollbar p-0.5",
        isMobile ? "w-full" : "w-72",
      )}
    >
...
function QRCodeDisplay(props: QRCodeDisplayProps) {
  ...
  return (
    <div className="relative inline-block">
      {/* @ts-ignore: qrcode.react resolves a different @types/react in CI; the JSX-component type mismatch is environment-dependent. */}
      <QRCode
        className={clsx(

  ...
  const handleSign = async () => {
    try {
      ...
      if (error instanceof ZeroXKeyError) onError(error);
        ...
        const tkError = new ZeroXKeyError(
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col mt-2 px-3 space-y-2">
        <Textarea
          ref={textareaRef}
          className="p-2 min-h-12 max-h-72 rounded-md zeroxkey-scrollbar border border-modal-background-dark/10 dark:border-modal-background-light/10 bg-icon-background-light dark:bg-icon-background-dark text-icon-text-light dark:text-icon-text-dark focus:outline-none resize-none overflow-y-auto"
          value={message}
          readOnly
        />
      </div>

...
import { ClientState, type ZeroXKeyProviderConfig } from "../../types/base";
import { useZeroXKey } from "../client/Hook";
...
export function ModalRoot(props: ModalRootProps) {
  const { config } = props; // Note: This is the config passed into the ZeroXKeyProvider. If we ever need to get config from the dashboard as well, grab `config` from the useZeroXKey hook instead.
  const {
    modalStack,
    popPage,
    closeModal,
  ...
  const canClose = !root?.preventClose;
    ...
    <Transition appear show={!!current} as={Fragment}>
      ...
      <Portal as={Fragment}>
        ...
        <div className="relative z-40">
          ...
          <div
            className={clsx(
              "zeroxkey-modal fixed inset-0 flex justify-center transition-colors",
                ...
                >
                  ...
                  >
                    {clientState === ClientState.Error ? (
                      <DeveloperError
                        developerTitle="ZeroXKey SDK failed to initialize."
                        developerMessages={[
                          "Check your config and ensure you're connected to the internet.",
                          "Try attaching an onError callback to the ZeroXKeyProvider's callbacks to see more details.",
...
function useCssLoaded(modalStack: ModalPage[], config: ZeroXKeyProviderConfig) {
  useEffect(() => {
    if (process.env.NODE_ENV !== "development") return; // Only check in development mode

    const testElement = document.createElement("div");
    testElement.className = "zeroxkey-modal"; // Use a class that should definitely be styled
...
function renderMissingStylesOverlay() {
  ...
  if (document.getElementById("zeroxkey-missing-css")) return;

  const div = document.createElement("div");
  div.id = "zeroxkey-missing-css";
  ...
  div.innerHTML = `
    <div style="font-size: 1.5rem; font-weight: bold; margin-bottom: 1rem;">
      ⚠️ ZeroXKey styles are missing
    </div>
    <div style="font-size: 1rem; max-width: 600px;">
      You must import the ZeroXKey React Wallet Kit styles in your app:
      ...
      <button id="zeroxkey-missing-css-close" style="margin-top: 2rem; padding: 0.5rem 1.5rem; font-size: 1rem; background: #b91c1c; color: white; border: none; border-radius: 6px; cursor: pointer;">
        Close
      </button>
    </div>
  `;
  ...
  const closeBtn = div.querySelector("#zeroxkey-missing-css-close");
  if (closeBtn) {

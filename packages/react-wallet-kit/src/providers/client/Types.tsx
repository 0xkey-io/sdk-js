 ...
 * ClientContextType
 *
 * This is where we define the shape of the context provided by the ZeroXKeyProvider. JS doc comments MUST be added here to show up in the generated docs as well as intellisense in IDEs.
 ...
 * **IMPORTANT** - New core methods DO NOT need to be redeclared here, their shape and JS doc comments should come from the core package directly via ZeroXKeyClientMethods.
 */
...
export interface ClientContextType
  ...
  authState: AuthState;
  /** @internal */
  config?: ZeroXKeyProviderConfig | undefined;
  /** @internal */
   ...
   * @throws {ZeroXKeyError} If the client is not initialized or if there is an error refreshing the user details.
   */
   ...
   * @throws {ZeroXKeyError} If the client is not initialized or if there is an error refreshing the wallets.
   */
   ...
   * - Handles error cases such as missing configuration, popup failures, missing PKCE verifier, or ZeroXKey proxy failures, throwing a `ZeroXKeyError` with appropriate error codes.
   *
   ...
   * @throws {ZeroXKeyError} If the configuration is not ready, required parameters are missing, or if there is an error initiating or completing the OAuth flow.
   */
   ...
   * - On successful authentication, the function either calls the provided `onOauthSuccess` callback, triggers the `onOauthRedirect` callback from provider callbacks, or completes the OAuth flow internally by calling `completeOauth`.
   * - Handles all error cases, including missing configuration, popup failures, and timeouts, and throws a `ZeroXKeyError` with appropriate error codes.
   *
   ...
   * - Uses ZeroXKey's export iframe flow to securely export wallet material.
   ...
   * - A request is made to the ZeroXKey API to export the wallet, encrypted to the target public key.
   ...
   * - Uses ZeroXKey's export iframe flow to securely export private key material.
   * - The export process encrypts the exported bundle to a target public key, which is generated and managed inside the iframe for maximum security.
   * - A request is made to the ZeroXKey API to export the private key, encrypted to the target public key.
   * - The resulting export bundle is injected into the iframe, where it is decrypted and displayed to the user.
   * - If a custom iframe URL is used, a target public key can be provided explicitly.
   ...
   * - Uses ZeroXKey's export iframe flow to securely export wallet account material.
   * - The export process encrypts the exported bundle to a target public key, which is generated and managed inside the iframe for maximum security.
   * - A request is made to the ZeroXKey API to export the wallet account, encrypted to the target public key.
   * - The resulting export bundle is injected into the iframe, where it is decrypted and displayed to the user.
   * - If a custom iframe URL is used, a target public key can be provided explicitly.
   * - Hexadecimal and Solana address formats are supported for wallet account exports - defaulting to Hexadecimal if not specified.
   * - Optionally allows specifying the stamper to use for the export (StamperType.Passkey, StamperType.ApiKey, or StamperType.Wallet) for granular authentication control.
   ...
   * - Handles all error cases and throws a ZeroXKeyError with appropriate error codes.
   *
   ...
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, or if there is an error updating the user name.
   */
   ...
   * - After successful verification and update, the user details state is refreshed and an optional success page can be shown.
   * - Supports customizing the duration of the success page after update.
   * - Throws a ZeroXKeyError if the client is not initialized, no active session is found, SMS OTP is not enabled, or if there is an error updating the phone number.
   *
   ...
   *
   * @returns A promise that resolves to the userId of the user that was changed.
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, SMS OTP is not enabled, or if there is an error updating the phone number.
   */
   ...
   *
   * @returns A promise that resolves to the userId of the user that was changed.
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, or if there is an error updating the email.
   */
   ...
   *
   * @returns A promise that resolves to the userId of the user that was changed.
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, or if there is an error adding the email.
   */
   ...
   *
   * @returns A promise that resolves to the userId of the user that was changed.
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, or if there is an error adding the phone number.
   */
   ...
   *
   * @returns A void promise.
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, or if there is an error adding the provider.
   */
   ...
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found,
   ...
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, or if there is an error adding the passkey.
   */
   ...
   * @throws {ZeroXKeyError} If the client is not initialized, no active session is found, or if there is an error removing the passkey.
   */
   ...
   * - Supports signing with any wallet account managed by ZeroXKey, including externally connected wallets.
   ...
   * @throws {ZeroXKeyError} If the client is not initialized, if there is an error during the signing process, or if the user cancels the action.
   */
  handleSignMessage: (
  ...
  /**
   * Handles the connecting of an external wallet account to the user's ZeroXKey account.
   *
   ...
   * - Supports both Ethereum and Solana wallet providers, and can be extended to additional chains as supported by ZeroXKey.
   * - Handles all error cases and throws a ZeroXKeyError with appropriate error codes if the client is not initialized or no active session is found.
   *
   ...
   * @throws {ZeroXKeyError} If the client is not initialized or if the user cancels the action.
   */
  ...
  /**
   * Handles the removal of a user's email address from their ZeroXKey account.
   *
  ...
  /**
   * Handles the removal of a user's phone number from their ZeroXKey account.
   *
   ...
   *
   * - Submits a send-transaction intent to ZeroXKey for signing and execution.
   ...
   * @param params.organizationId - Optional ZeroXKey organization or sub-organization ID
   ...
   * @throws {ZeroXKeyError} If submission fails, polling fails, or the user cancels the flow.
   */
   ...
   *
   * - Submits an ERC20 transfer intent to ZeroXKey for signing and execution.
   * - Automatically polls the transaction status until it reaches a terminal state.
   * - Displays a modal showing progress and a success page upon completion.
   *

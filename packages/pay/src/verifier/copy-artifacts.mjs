...
const manifest = new Uint8Array(
  readFileSync(path.join(pr002Root, "bundle-manifest.json")),
);
const files = Object.fromEntries(
  readdirSync(pr002Root)
    ...
    .map((file) => [
      file,
      new Uint8Array(readFileSync(path.join(pr002Root, file))),
    ]),
);
...
for (const [file, expected] of [
  [
    "eip3009-wrong-signer.json",
    sdk.COMMERCE_VERIFIER_METADATA.wrongSignerVectorDigest,
  ],
  [
    "conformance-v1.json",
    sdk.COMMERCE_VERIFIER_METADATA.conformanceCorpusDigest,
  ],
  [
    "conformance-report.jsonl",
    sdk.COMMERCE_VERIFIER_METADATA.conformanceReportDigest,
  ],
]) {
  const actual = `sha256:${createHash("sha256")
    .update(readFileSync(path.join(sourceRoot, file)))
    .digest("hex")}`;

...
function highSJws(envelope: any): any {
  ...
  return {
    ...envelope,
    signature: `${header}.${payload}.${Buffer.concat([raw.subarray(0, 32), high]).toString("base64url")}`,
  };
}
...
async function runCase(operation: string): Promise<CommerceVerificationResult> {
  if (operation.startsWith("JWS_")) {
    ...
    if (operation === "JWS_CUSTODY_VALID") {
      registry = registry.map((key, index) =>
        index === 0 ? { ...key, role: "CUSTODY" } : key,
      ) as HistoricalCommerceKey[];
      ...
      registry = registry.map((key, index) =>
        index === 0 ? { ...key, notBeforeUnixMs: 4_000_000_000_000 } : key,
      );
    } else if (operation === "JWS_JWK_KID_MISMATCH") {
      registry = registry.map((key, index) =>
        index === 0 ? { ...key, jwk: { ...key.jwk, kid: "wrong-key" } } : key,
      );
    }
    const envelope =
      operation === "JWS_ISSUER_MISMATCH"
  ...
  if (operation.startsWith("EIP3009_")) {
    const envelope =
      operation === "EIP3009_CORRUPT_SIGNATURE"
        ? {
            ...eip.envelope,
            signature: `0x9${eip.envelope.signature.slice(3)}`,
          }
  ...
  if (
    operation === "BOOT_EPHEMERAL_MISMATCH" ||
    operation === "BOOT_PIVOT_MISMATCH"
  ) {
    const appProof =
      operation === "BOOT_EPHEMERAL_MISMATCH"
        ? {
            ...boot.appProof,
            publicKey: boot.appProof.publicKey.replace(
              /.$/,
              boot.appProof.publicKey.endsWith("A") ? "B" : "A",
            ),
          }
        : boot.appProof;
    const bootProof =
      operation === "BOOT_PIVOT_MISMATCH"
        ? {
            ...boot.bootProof,
            qosManifestB64: boot.bootProof.qosManifestB64.replace(
              /^./,
              boot.bootProof.qosManifestB64.startsWith("A") ? "B" : "A",
            ),
          }
        : boot.bootProof;
...
it("emits the byte-pinned ordered cross-language conformance report without networking", async () => {
  ...
  globalThis.fetch = (() => {
    throw new Error("network access from public verifier");
  }) as typeof fetch;
  try {
    const records = [];
    for (const fixture of corpus.cases as Array<{
      id: string;
      operation: string;
    }>) {

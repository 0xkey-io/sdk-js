...
import { TERMINAL_ACTIVITY_STATUSES, TActivityResponse, TActivityStatus, TSignedRequest } from "@0xkey-io/http";

import type { definitions } from "../__inputs__/public_api.types";

import { GrpcStatus, TStamper, ZeroXKeyRequestError, ZeroXKeySDKClientConfig } from "../__types__/base";
...
export class ZeroXKeySDKClientBase {
  ...
  async activityDecision<TBodyType, TResponseType>(
	...
	tronSendTransaction = async (input: SdkApiTypes.TTronSendTransactionBody): Promise<SdkApiTypes.TTronSendTransactionResponse> => {
    const { organizationId, timestampMs, ...rest } = input;
    return this.command("/public/v1/submit/tron_send_transaction", {
      parameters: rest,
      organizationId: organizationId ?? this.config.organizationId,
      timestampMs: timestampMs ?? String(Date.now()),
	...
	stampTronSendTransaction = async (input: SdkApiTypes.TTronSendTransactionBody): Promise<TSignedRequest | undefined> => {
    if (!this.stamper) {
      return undefined;
    }

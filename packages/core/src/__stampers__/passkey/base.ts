...
let PasskeyStamperModule: typeof import("@0xkey-io/react-native-passkey-stamper");
...
export class CrossPlatformPasskeyStamper implements TStamper {
  ...
  async init(): Promise<void> {
    if (isWeb()) {
      ...
      try {
        ...
        let PasskeyStamper;
        try {
          PasskeyStamperModule = require("@0xkey-io/react-native-passkey-stamper");
          PasskeyStamper = PasskeyStamperModule.PasskeyStamper;
        } catch {
          throw new Error(
            "Please install react-native-passkeys and @0xkey-io/react-native-passkey-stamper in your app to use passkeys.",
          );
        }
  ...
  createReactNativePasskey = async (
    config: Record<any, any> = {}, // TODO (Amir): This needs to be typed
  ): Promise<ZeroXKeyAuthenticatorParams> => {
    ...
    if (!createPasskey) {
      throw new Error(
        "Ensure you have @0xkey-io/react-native-passkey-stamper installed and linked correctly. Are you not on React Native?",
      );
    }
    ...
    return await createPasskey({
      rp: {
        id: this.config.rpId!,
        name: this.config.rpName ?? "ZeroXKey",
      },
      user: {
        id: uuidv4(),
        name,

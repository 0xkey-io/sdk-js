...
const utf8Decode = (value: Uint8Array): string =>
  new TextDecoder().decode(value);

const encryptOtpAttemptBundle = ({
  otpCode,
  publicKey,
  ...
  const targetBundle = JSON.parse(otpEncryptionTargetBundle) as {
    data: string;
  };
  const targetData = JSON.parse(utf8Decode(hexToBytes(targetBundle.data))) as {
    targetPublic: string;
  };
  ...
  const encrypted = hpkeEncrypt({
    ...
    targetKeyBuf: hexToBytes(targetData.targetPublic),
    hpkeInfo: "0xkey_hpke",
  });
...
export class ZeroXKeyClient {
  ...
  /**
   * Creates a new ZeroXKeySDKClientBase instance with the provided configuration.
   ...
   *
   * @returns A new instance of {@link ZeroXKeySDKClientBase} configured with the provided parameters.
   */
  createHttpClient = (
   ...
   * - The resulting attestation and challenge can be used to register the passkey with ZeroXKey.
   *
   ...
   * @throws {ZeroXKeyError} If there is an error during passkey creation, or if the platform is unsupported.
   */
  createPasskey = async (
   ...
   * @throws {ZeroXKeyError} If there is no active session or if there is an error during the logout process.
   */
  logout = async (params?: LogoutParams): Promise<void> => {
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.LOGOUT_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is an error during the passkey login process or if the user cancels the passkey prompt.
   */
  loginWithPasskey = async (
    ...
    return await withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.PASSKEY_LOGIN_AUTH_ERROR,
        customErrorsByMessages: {
          "timed out or was not allowed": {
   ...
   * @throws {ZeroXKeyError} If there is an error during passkey creation, sub-organization creation, or session storage.
   */
  signUpWithPasskey = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      {
        errorCode: ZeroXKeyErrorCodes.PASSKEY_SIGNUP_AUTH_ERROR,
   ...
   * @throws {ZeroXKeyError} If the wallet manager is uninitialized or provider retrieval fails.
   */
  fetchWalletProviders = async (chain?: Chain): Promise<WalletProvider[]> => {
    return withZeroXKeyErrorHandling(
      async () => {
        if (!this.walletManager) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.WALLET_MANAGER_COMPONENT_NOT_INITIALIZED,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.FETCH_WALLETS_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If the wallet manager is uninitialized or the connection fails.
   */
  connectWalletAccount = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.CONNECT_WALLET_ACCOUNT_ERROR,
        customErrorsByMessages: {
          "WalletConnect: The connection request has expired. Please scan the QR code again.":
            {
              ...
              code: ZeroXKeyErrorCodes.WALLET_CONNECT_EXPIRED,
            },
          "User rejected the request": {
            message: "Connect wallet was cancelled by the user.",
            code: ZeroXKeyErrorCodes.CONNECT_WALLET_CANCELLED,
          },
        },
      },
    );
   ...
   * @throws {ZeroXKeyError} If the wallet manager is uninitialized or the disconnection fails.
   */
  disconnectWalletAccount = async (walletProvider: WalletProvider) => {
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.DISCONNECT_WALLET_ACCOUNT_ERROR,
      },
    );
  };
   ...
   *
   * @throws {ZeroXKeyError} If the wallet manager is uninitialized, the provider is not connected, or the switch fails.
   */
  switchWalletAccountChain = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (!walletProvider) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.SWITCH_WALLET_CHAIN_ERROR,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.SWITCH_WALLET_CHAIN_ERROR,
      },
    );
  };

  /**
   * Builds and signs a wallet login request without submitting it to ZeroXKey.
   *
   * - This function prepares a signed request for wallet authentication, which can later be used
   *   to log in or sign up a user with ZeroXKey.
   ...
   * - The signed request is not sent to ZeroXKey immediately; it is meant to be used in a subsequent flow
   ...
   * @throws {ZeroXKeyError} If the wallet stamper is not initialized, the signing process fails,
      ...
      async () => {
        ...
        if (!futureSessionPublicKey) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.WALLET_BUILD_LOGIN_REQUEST_ERROR,
          );
        }
        ...
        // here we sign the request with the wallet, but we don't send it to ZeroXKey yet
        // this is because we need to check if the subOrg exists first, and create one if it doesn't
        // once we have the subOrg for the publicKey, we then can send the request to ZeroXKey
        const signedRequest = await withZeroXKeyErrorHandling(
          async () => {
          ...
          },
          {
            ...
            errorCode: ZeroXKeyErrorCodes.WALLET_BUILD_LOGIN_REQUEST_ERROR,
            customErrorsByMessages: {
              "WalletConnect: The connection request has expired. Please scan the QR code again.":
                {
                  message:
   ...
   * @throws {ZeroXKeyError} If the wallet stamper is uninitialized, a public key cannot be found or generated, or login fails.
   */
  loginWithWallet = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.WALLET_LOGIN_AUTH_ERROR,
      },
    );
  };
   ...
   *          - `sessionToken`: the signed JWT session token.
   *          - `address`: the authenticated wallet address.
   * @throws {ZeroXKeyError} If there is an error during wallet authentication, sub-organization creation, session storage, or cleanup.
   */
  signUpWithWallet = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        // now we can send the stamped request to ZeroXKey
        const sessionResponse =
          await this.httpClient.sendSignedRequest<TStampLoginResponse>(
            signedRequest,
          );
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.WALLET_SIGNUP_AUTH_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is an error during wallet authentication, sub-organization creation, or session storage.
   */
  loginOrSignupWithWallet = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      {
        errorCode: ZeroXKeyErrorCodes.WALLET_LOGIN_OR_SIGNUP_ERROR,
      ...
      async () => {
        ...
        if (
          ...
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.INIT_OTP_ERROR,
          );
        }
  ...
  verifyOtp = async (params: VerifyOtpParams): Promise<VerifyOtpResult> => {
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        const encryptedOtpBundle = encryptOtpAttemptBundle({
          otpCode,
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.VERIFY_OTP_ERROR,
        customErrorsByMessages: {
          "Invalid OTP code": {
            message: "The provided OTP code is invalid.",
            code: ZeroXKeyErrorCodes.INVALID_OTP_CODE,
          },
        },
      },
    );
   ...
   * @returns A promise that resolves to a {@link BaseAuthResult}, which includes:
   *          - `sessionToken`: the signed JWT session token.
   * @throws {ZeroXKeyError} If there is an error during the OTP sign-up process or session storage.
   */
  signUpWithOtp = async (
   ...
   *          - `action`: whether the flow resulted in a login or signup ({@link AuthAction}).
   * @throws {ZeroXKeyError} If there is an error during OTP verification, sign-up, or login.
   */
  completeOtp = async (
  ...
  > => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.OTP_COMPLETION_ERROR,
      },
    );
  };
   ...
   *          - `sessionToken`: the signed JWT session token.
   *          - `action`: whether the flow resulted in a login or signup ({@link AuthAction}).
   * @throws {ZeroXKeyError} If there is an error during the OAuth completion process, such as account lookup, sign-up, or login.
   */
  completeOauth = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.OAUTH_LOGIN_ERROR,
      },
    );
  };
   ...
   * @returns A promise that resolves to a {@link BaseAuthResult}, which includes:
   *          - `sessionToken`: the signed JWT session token.
   * @throws {ZeroXKeyError} If there is an error during the OAuth sign-up or login process.
   */
  signUpWithOauth = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.OAUTH_SIGNUP_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If no active session is found or if there is an error fetching wallets.
   */
   ...
   *
   * - For embedded wallets, retrieves accounts from the ZeroXKey API, supporting pagination (defaults to the first page with a limit of 100 accounts).
   ...
   * @throws {ZeroXKeyError} If no active session is found or if there is an error fetching wallet accounts.
   */
  fetchWalletAccounts = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        // this is an embedded wallet so we fetch accounts from ZeroXKey
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.FETCH_WALLET_ACCOUNTS_ERROR,
      },
    );
  };
   ...
   *
   * - Retrieves private keys from the ZeroXKey API.
   * - Supports stamping the request with a specific stamper (StamperType.Passkey, StamperType.ApiKey, or StamperType.Wallet).
   *
   * @param params.stampWith - parameter to stamp the request with a specific stamper (StamperType.Passkey, StamperType.ApiKey, or StamperType.Wallet).
   * @param params.organizationId - organization ID to target (defaults to the session's organization ID).
   * @returns A promise that resolves to an array of `v1PrivateKey` objects.
   * @throws {ZeroXKeyError} If no active session is found or if there is an error fetching private keys.
   */
  fetchPrivateKeys = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.FETCH_PRIVATE_KEYS_ERROR,
      },
    );
  };
   ...
   *
   * - **Embedded wallets**
   *   - Uses the ZeroXKey API to sign the message directly.
   ...
   * @throws {ZeroXKeyError} If signing fails, the wallet type does not support message signing, or the response is invalid.
   */
  signMessage = async (
    ...
    return withZeroXKeyErrorHandling(
        ...
        if (walletAccount.source === WalletSource.Connected) {
          ...
          if (!addEthereumPrefix && isEthereum) {
            throw new ZeroXKeyError(
              ...
              ZeroXKeyErrorCodes.SIGN_MESSAGE_ERROR,
            );
          }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.SIGN_MESSAGE_ERROR,
      },
    );
  };
   ...
   *
   * - **Embedded wallets**
   *   - Delegates signing to the ZeroXKey API, which returns the signed transaction.
   *   - Supports all ZeroXKey-supported transaction types (e.g., Ethereum/EVM, Solana, Tron).
   *   - BSC and other EVM-compatible chains use `TRANSACTION_TYPE_ETHEREUM`; the chain is identified by the unsigned transaction's `chainId`.
   ...
   * @param params.transactionType - type of transaction (e.g., "TRANSACTION_TYPE_ETHEREUM", "TRANSACTION_TYPE_SOLANA", "TRANSACTION_TYPE_TRON"). Use "TRANSACTION_TYPE_ETHEREUM" for BSC.
   ...
   * @throws {ZeroXKeyError} If the wallet type is unsupported, signing fails, or the response is invalid.
   */
  signTransaction = async (params: SignTransactionParams): Promise<string> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.SIGN_TRANSACTION_ERROR,
      },
    );
  };
   ...
   *
   * - **Embedded wallets**
   *   - Signs the transaction using the ZeroXKey API.
   *   - Requires an `rpcUrl` to broadcast the signed transaction, since ZeroXKey does not broadcast directly.
   ...
   * @param params.organizationId - **Only for ZeroXKey embedded wallets**: organization ID to target (defaults to the session's organization ID).
   * @returns A promise that resolves to a transaction signature or hash.
   * @throws {ZeroXKeyError} If the wallet type is unsupported, or if signing/broadcasting fails.
   */
  signAndSendTransaction = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
            ...
            case Chain.Solana:
              if (!rpcUrl) {
                throw new ZeroXKeyError(
                  ...
                  ZeroXKeyErrorCodes.SIGN_AND_SEND_TRANSACTION_ERROR,
                );
              }
              // not sure why typescript isn't inferring the type here
              // if namespace is Chain.Solana, then it must be a ConnectedSolanaWalletAccount
        ...
        // since ZeroXKey does not broadcast transactions directly
        if (!rpcUrl) {
          throw new ZeroXKeyError(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.SIGN_AND_SEND_TRANSACTION_ERROR,
      },
    );
  };
   ...
   * using a ZeroXKey-managed (embedded) wallet.
   *
   ...
   * @throws {ZeroXKeyError} If amount encoding fails or ZeroXKey rejects the transaction.
   */
  ethSendErc20Transfer = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.ETH_SEND_TRANSACTION_ERROR,
      },
    );
  };
   ...
   * * **API subject to change**
   *
   * Signs and submits an Ethereum transaction using a ZeroXKey-managed (embedded) wallet.
   *
   * This method performs **authorization and signing**, and submits the transaction
   * to ZeroXKey’s coordinator. It **does not perform any polling** — callers must use
   ...
   *
   * - **Embedded wallets**
   *   - Constructs the payload for ZeroXKey's `eth_send_transaction` endpoint.
   *   - Forwards transaction fields directly to ZeroXKey's coordinator.
   *   - Signs and submits the transaction through ZeroXKey.
   ...
   *
   * @throws {ZeroXKeyError} If the transaction is invalid or ZeroXKey rejects it.
   */
      ...
      async () => {
        ...
        if (!id) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.ETH_SEND_TRANSACTION_ERROR,
          );
        }
   ...
   * * **API subject to change**
   *
   * Signs and submits a Solana transaction using a ZeroXKey-managed (embedded) wallet.
   *
   * This method performs **authorization and signing**, and submits the transaction
   * to ZeroXKey’s coordinator. It **does not perform any polling** — callers must use
   * `pollTransactionStatus` to obtain the final on-chain result.
   ...
   *
   * - **Embedded wallets**
   *   - Constructs the payload for ZeroXKey's `sol_send_transaction` endpoint.
   *   - Signs and submits the transaction through ZeroXKey.
   *   - Returns a `sendTransactionStatusId`, which the caller must pass to
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (!id) {
          throw new ZeroXKeyError(
            "Missing sendTransactionStatusId",
            ZeroXKeyErrorCodes.SOL_SEND_TRANSACTION_ERROR,
          );
        }
   ...
   * **API subject to change**
   *
   * Polls ZeroXKey for the final result of a previously submitted transaction.
   *
   ...
   * Behavior:
   *
   * - Queries ZeroXKey every 500ms.
   * - Stops polling automatically when a terminal state is reached.
   * - Returns the full status payload from ZeroXKey.
   ...
   * @throws {ZeroXKeyError} If the transaction fails, is cancelled, or times out.
   */
  ...
  async pollTransactionStatus(
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.POLL_TRANSACTION_STATUS_ERROR,
      },
    );
  }
   ...
   *
   * - Retrieves user details from the ZeroXKey API using the provided userId and organizationId, or defaults to those from the active session.
   ...
   * @throws {ZeroXKeyError} If there is no active session, if there is no userId, or if there is an error fetching user details.
   */
  fetchUser = async (params?: FetchUserParams): Promise<v1User> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.FETCH_USER_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the input is invalid, if user retrieval fails, or if user creation fails.
   */
  fetchOrCreateP256ApiKeyUser = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (
          ...
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.CREATE_USERS_ERROR,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.CREATE_USERS_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the input is invalid,
      ...
      async () => {
        ...
        if (!createPoliciesResponse || !createPoliciesResponse.policyIds) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.CREATE_POLICY_ERROR,
          );
        }
        ...
        const newlyCreatedPolicies = missingPolicies.map((p, idx) => ({
          ...
          // we can safely assert the ID exists because we know ZeroXKey's api
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the userId is missing, or if there is an error updating or verifying the user email.
   */
  updateUserEmail = async (params: UpdateUserEmailParams): Promise<string> => {
    ...
    return withZeroXKeyErrorHandling(
        ...
        if (verificationToken) {
          ...
          if (existingUser.organizationId) {
            throw new ZeroXKeyError(
              ...
              ZeroXKeyErrorCodes.ACCOUNT_ALREADY_EXISTS,
            );
          }
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.UPDATE_USER_EMAIL_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the userId is missing, or if there is an error removing the user email.
   */
   ...
   * @param params.organizationId - organization ID to specify the sub-organization (defaults to the current session's organizationId).
   * @returns A promise that resolves to the userId of the updated user.
   * @throws {ZeroXKeyError} If there is no active session, if the userId is missing, or if there is an error updating or verifying the user phone number.
   */
  updateUserPhoneNumber = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (!res || !res.userId) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.UPDATE_USER_PHONE_NUMBER_ERROR,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.UPDATE_USER_PHONE_NUMBER_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the userId is missing, or if there is an error removing the user phone number.
   */
   ...
   * @param params.organizationId - organization ID to specify the sub-organization (defaults to the current session's organizationId).
   * @returns A promise that resolves to the userId of the updated user.
   * @throws {ZeroXKeyError} If there is no active session, if the userId is missing, or if there is an error updating the user name.
   */
  updateUserName = async (params: UpdateUserNameParams): Promise<string> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.UPDATE_USER_NAME_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the account already exists, or if there is an error adding the OAuth provider.
   */
  addOauthProvider = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        // parse the oidc token so we can get the email. Pass it in to updateUser then call createOauthProviders. This will be verified by ZeroXKey.
        ...
        if (!createProviderRes) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.ADD_OAUTH_PROVIDER_ERROR,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.ADD_OAUTH_PROVIDER_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the userId is missing, or if there is an error removing the OAuth provider.
   */
  removeOauthProviders = async (
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (!res) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.REMOVE_OAUTH_PROVIDER_ERROR,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.REMOVE_OAUTH_PROVIDER_ERROR,
      },
    );
  };
   ...
   * - The resulting passkey attestation and challenge are registered with ZeroXKey as a new authenticator.
   *
   * @param params.name - name of the passkey (defaults to "ZeroXKey Passkey-`timestamp`").
   ...
   * @throws {ZeroXKeyError} If there is no active session, if passkey creation fails, or if there is an error adding the passkey.
   */
  addPasskey = async (params?: AddPasskeyParams): Promise<string[]> => {
    const { stampWith = this.config.defaultStamperType, organizationId } =
      params || {};
    const name = params?.name || `ZeroXKey Passkey-${Date.now()}`;
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (!attestation || !encodedChallenge) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.CREATE_PASSKEY_ERROR,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.ADD_PASSKEY_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the userId is missing, or if there is an error removing the passkeys.
   */
  removePasskeys = async (params: RemovePasskeyParams): Promise<string[]> => {
    ...
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (!res) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.REMOVE_PASSKEY_ERROR,
          );
        }
        return res.authenticatorIds;
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.REMOVE_PASSKEY_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session or if there is an error creating the wallet.
   */
  createWallet = async (params: CreateWalletParams): Promise<string> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.CREATE_WALLET_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the wallet does not exist, or if there is an error creating the wallet accounts.
   */
  createWalletAccounts = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.CREATE_WALLET_ACCOUNT_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the targetPublicKey is missing, or if there is an error exporting the wallet.
   */
  exportWallet = async (params: ExportWalletParams): Promise<ExportBundle> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.EXPORT_WALLET_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the targetPublicKey is missing, or if there is an error exporting the private key.
   */
  exportPrivateKey = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.EXPORT_PRIVATE_KEY_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the targetPublicKey is missing, or if there is an error exporting the wallet account.
   *
   */
  exportWalletAccount = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.EXPORT_WALLET_ACCOUNT_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the encrypted bundle is invalid, or if there is an error importing the wallet.
   */
  importWallet = async (params: ImportWalletParams): Promise<string> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.IMPORT_WALLET_ERROR,
        customErrorsByMessages: {
          "invalid mnemonic": {
   ...
   * @throws {ZeroXKeyError} If there is no active session or if there is an error deleting the sub-organization.
   */
  deleteSubOrganization = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.DELETE_SUB_ORGANIZATION_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If the session does not exist or if there is an error clearing the session.
   */
  clearSession = async (params?: ClearSessionParams): Promise<void> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.CLEAR_SESSION_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If no sessions exist or if there is an error clearing all sessions.
   */
  clearAllSessions = async (): Promise<void> => {
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.CLEAR_ALL_SESSIONS_ERROR,
      },
    );
  };
   ...
   * - Makes a request to the ZeroXKey API to stamp a new login and stores the refreshed session token.
   ...
   * @throws {ZeroXKeyError} If the session key does not exist, if there is no active session, or if there is an error refreshing the session.
   */
  refreshSession = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.REFRESH_SESSION_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is an error retrieving the session from storage.
   */
  getSession = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.GET_SESSION_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is an error retrieving sessions from storage.
   */
  getAllSessions = async (): Promise<Record<string, Session> | undefined> => {
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.GET_ALL_SESSIONS_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If the client is not initialized or if there is an error setting the active session key.
   */
  setActiveSession = async (params: SetActiveSessionParams): Promise<void> => {
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.SET_ACTIVE_SESSION_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is an error retrieving the active session key from storage.
   */
  getActiveSessionKey = async (): Promise<string | undefined> => {
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.GET_ACTIVE_SESSION_KEY_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If the API key stamper is not initialized or if there is an error during key pair creation or storage.
   */
  createApiKeyPair = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.CREATE_API_KEY_PAIR_ERROR,
      },
    );
  };
   ...
   *
   * - This function makes a request to the ZeroXKey auth proxy to retrieve the current WalletKit configuration,
   ...
   * @throws {ZeroXKeyError} If there is an error retrieving the proxy authentication configuration from the auth proxy.
   */
  getProxyAuthConfig = async (): Promise<ProxyTGetWalletKitConfigResponse> => {
    return withZeroXKeyErrorHandling(
      async () => {
        ...
        if (!res) {
          throw new ZeroXKeyError(
            ...
            ZeroXKeyErrorCodes.GET_PROXY_AUTH_CONFIG_ERROR,
          );
        }
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.GET_PROXY_AUTH_CONFIG_ERROR,
      },
    );
  };
  ...
  fetchBootProofForAppProof = async (
    ...
    return withZeroXKeyErrorHandling(
      ...
      },
      {
        ...
        errorCode: ZeroXKeyErrorCodes.FETCH_BOOT_PROOF_ERROR,
      },
    );
  };
   ...
   * @throws {ZeroXKeyError} If there is no active session, if the input is invalid, or if verification fails.
   */

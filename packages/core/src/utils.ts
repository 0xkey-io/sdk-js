...
import { fromDerSignature, uncompressRawPublicKey } from "@0xkey-io/crypto";
import {
 ...
 * ```ts
 * // Example usage:
 * import { addressFormatConfig } from "@0xkey-io/sdk-core";
 *
...
const globalErrorsToMatch: Readonly<
  Record<string, { message: string; code: ZeroXKeyErrorCodes }>
> = Object.freeze({
...
export const addressFormatConfig: Partial<
  Record<v1AddressFormat, AddressFormatConfig>
> = {
...
export function splitSignature(
  ...
  if (addressFormat === "ADDRESS_FORMAT_ETHEREUM") {
    ...
    // so we return 00 to match what ZeroXKey does
    const v = "00";
    return { r, s, v };
  }
...
/**
 * Extracts the public key from a ZeroXKey stamp header value.
...
/**
 * Submits a signed request to ZeroXKey.
 *
 ...
 * @returns The parsed JSON response from ZeroXKey.
 * @throws ZeroXKeyNetworkError if the request fails.
 */
...
export async function sendSignedRequest<T = any>(
  ...
  if (!res.ok) {
    ...
    throw new ZeroXKeyNetworkError(
...
export async function withZeroXKeyErrorHandling<T>(
  ...
  catchOptions: {
    ...
    customErrorsByCodes?: Partial<
      Record<ZeroXKeyErrorCodes, { message: string; code: ZeroXKeyErrorCodes }>
    >;
    customErrorsByMessages?: Record<
      string,
      { message: string; code: ZeroXKeyErrorCodes }
    >;
  },
  finallyOptions?: {
  ...
  try {
    ...
    // we normalize here to always produce a readable error message before wrapping it in ZeroXKeyError.
    ...
    if (error instanceof ZeroXKeyError) {
      ...
      throw error;
    } else if (error instanceof ZeroXKeyRequestError) {
      throwMatchingMessage(normalizedMessage, customErrorsByMessages, error);

      throw new ZeroXKeyError(errorMessage, errorCode, error);
    } else if (error instanceof Error) {
      throwMatchingMessage(normalizedMessage, customErrorsByMessages, error);

      // Wrap other errors in a ZeroXKeyError
      throw new ZeroXKeyError(errorMessage, errorCode, error);
    } else {
      throwMatchingMessage(normalizedMessage, customErrorsByMessages, error);
      // Handle non-Error exceptions
      throw new ZeroXKeyError(normalizedMessage, errorCode, error);
    }
  } finally {
    await finallyFn?.();
  }
...
/**
 * Throws a ZeroXKeyError with a custom message if the error message matches any key in customMessageByMessages.

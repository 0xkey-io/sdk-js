/// <reference lib="dom" />
// ZeroXKey-specific cryptographic utilities
import { bs58check } from "@0xkey-io/encoding";
...
// HPKE info string for the 0xkey-native key schedule (enclave import/export,
// credential bundles, on-ramp secrets). When hpkeInfo is omitted,
// getHpkeScheduleInfo falls back to the legacy schedule and ciphertext will not
// interoperate with 0xkey services; all hpkeEncrypt/hpkeDecrypt calls here must
// pass this value explicitly.
...
/**
 * Verifies a signature from a ZeroXKey stamp using ECDSA and SHA-256.
 *
 ...
 * @param {string} signedData - The data that was signed (e.g. JSON-stringified ZeroXKey request body).
 * @returns {Promise<boolean>} - Returns true if the signature is valid, otherwise throws an error.
 *
 * @example
 *
 * const stampedRequest = await 0xkeyClient.stampGetWhoami(...);
...
export const encryptPrivateKeyToBundle = async ({
  ...
  const privateKeyBundle = hpkeEncrypt({
    plainTextBuf,
    targetKeyBuf,
    hpkeInfo: ZEROXKEY_HPKE_INFO,
  });
...
/**
 * Verifies that a **session JWT** was signed by ZeroXKey’s
 ...
 * credentials to ZeroXKey via the CreateFiatOnRampCredential activity
 *

import { ZeroXKeyActivityError } from "@0xkey-io/http";
import {
  type LocalAccount,
  type Chain,
  type Hex,
...
const testCase: typeof test = (...argList) => {
  if (!process.env.BANNED_TO_ADDRESS) {
    // For now, this test requires certain environment variables to be injected (from either ZeroXKey's internal environment, or a local `.env` file)
...
describe("ZeroXKeyAccount", () => {
  let walletClient: any;
  let zeroXKeyAccount: LocalAccount;
  ...
  [
    ...
    describe(`using config ${signingConfig.configName}`, () => {
      beforeEach(async () => {
        ...
        zeroXKeyAccount = signingConfig.configName.includes(
      ...
      testCase("it is connected to the right address", async () => {
        ...
        expect(zeroXKeyAccount.address).toEqual(
        ...
        async () => {
          try {
            ...
            const { message, cause, activityId, activityStatus, activityType } =
              error as ZeroXKeyActivityError;
            ...
            }).toMatchInlineSnapshot(`
            {
              ...
              "activityStatus": null,
              "activityType": null,
              "cause": [ZeroXKeyRequestError: ZeroXKey error 7: policy engine denied request explicitly (Details: [])],
              "message": "Failed to sign: ZeroXKey error 7: policy engine denied request explicitly (Details: [])",
            }
          `);
          }
        },

...
type TConfig = {
  ...
  /**
   * ZeroXKey wallet account address, private key address, or private key ID
   */
  signWith: string;
};

export class ZeroXKeySigner extends AbstractSigner implements ethers.Signer {
  ...
  connect(provider: Provider): ZeroXKeySigner {
    return new ZeroXKeySigner(
      {
        client: this.client,
  ...
  // Otherwise, if it's a ZeroXKey Private Key ID, fetch the corresponding
  ...
  private async _signTransactionWithErrorWrapping(
    ...
    try {
      ...
      if (
        error instanceof ZeroXKeyActivityError ||
        error instanceof ZeroXKeyActivityConsensusNeededError
      ) {
        throw error;
      }
...
export { ZeroXKeyActivityError, ZeroXKeyRequestError };

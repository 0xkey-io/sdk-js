...
import { ZeroXKeySigner, ZeroXKeyActivityError } from "../";
...
const testCase: typeof test = (...argList) => {
  if (!process.env.BANNED_TO_ADDRESS) {
    // For now, this test requires certain environment variables to be injected (from ZeroXKey's internal environment, or a local `.env` file)
...
describe("ZeroXKeySigner", () => {
  let connectedSigner: ZeroXKeySigner;
  let signerWithProvider: ZeroXKeySigner;
  let chainId: bigint;
  ...
  [
    ...
    describe(`using config ${signingConfig.configName}`, () => {
      beforeEach(async () => {
        ...
        connectedSigner = new ZeroXKeySigner({
          client: zeroXKeyClient,
          organizationId,
          signWith: signingConfig.signWith,
        }).connect(provider);

        signerWithProvider = new ZeroXKeySigner(
          {
            client: zeroXKeyClient,
            organizationId,
            signWith: signingConfig.signWith,

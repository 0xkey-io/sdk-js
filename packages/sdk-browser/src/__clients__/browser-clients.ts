import type { WalletInterface } from "@0xkey-io/wallet-stamper";
import type { IframeStamper, KeyFormat } from "@0xkey-io/iframe-stamper";
import type { WebauthnStamper } from "@0xkey-io/webauthn-stamper";
import type { IndexedDbStamper } from "@0xkey-io/indexed-db-stamper";
import { getWebAuthnAttestation } from "@0xkey-io/http";
import { Session, SessionType } from "@0xkey-io/sdk-types";

import type * as SdkApiTypes from "../__generated__/sdk_api_types";
import { ZeroXKeyBaseClient } from "../__clients__/base-client";

import {
  AuthClient,
  ZeroXKeySDKClientConfig,
  ZeroXKeyWalletClientConfig,
...
import { uint8ArrayToHexString, pointEncode } from "@0xkey-io/encoding";
...
export class ZeroXKeyBrowserClient extends ZeroXKeyBaseClient {
   ...
   *
   * - For `READ_ONLY` sessions: Requires the client to be a `ZeroXKeyPasskeyClient`.
   * - For `READ_WRITE` sessions:
   *   - If the client is a `ZeroXKeyIndexedDbClient`, a new keypair will be generated unless a `publicKey` is provided.
   *   - If the client is a `ZeroXKeyIframeClient`, it will use the provided `publicKey` if available, or fall back to `getEmbeddedPublicKey()`.
    ...
    try {
      switch (sessionType) {
        case SessionType.READ_ONLY: {
          if (!(this instanceof ZeroXKeyPasskeyClient)) {
            throw new Error(
        ...
        case SessionType.READ_WRITE: {
          ...
          if (this instanceof ZeroXKeyIndexedDbClient) {
          ...
          if (this instanceof ZeroXKeyIframeClient) {
  ...
  /**
   * Log in with a session returns from ZeroXKey. This method uses a session from server actions and stores it and the active client in local storage
...
export class ZeroXKeyPasskeyClient extends ZeroXKeyBrowserClient {
  rpId: string;
  ...
  createUserPasskey = async (
    ...
    // ZeroXKey only supports ES256 (-7) and RS256 (-257)
    //
...
/**
 * ZeroXKeyIframeClient is a client that uses an iframe to interact with the ZeroXKey API.
 ...
 * @extends ZeroXKeyBrowserClient
 */
export class ZeroXKeyIframeClient extends ZeroXKeyBrowserClient {
...
export class ZeroXKeyWalletClient extends ZeroXKeyBrowserClient {
  private wallet: WalletInterface;

  constructor(config: ZeroXKeyWalletClientConfig) {
...
/**
 * ZeroXKeyIndexedDbClient is a client that uses IndexedDb to interact with the ZeroXKey API.
 * @extends ZeroXKeyBrowserClient
 */
export class ZeroXKeyIndexedDbClient extends ZeroXKeyBrowserClient {
  constructor(config: ZeroXKeySDKClientConfig) {

import { WalletStamper, type WalletInterface } from "@0xkey-io/wallet-stamper";
...
export class ZeroXKeyBrowserSDK {
  config: ZeroXKeySDKBrowserConfig;

  protected stamper: Stamper | undefined;

  constructor(config: ZeroXKeySDKBrowserConfig) {
    this.config = config;
  }

  /**
   * Creates a passkey client. The parameters override the default values passed to the underlying ZeroXKey `WebauthnStamper`
   * @param PasskeyClientParams
   * @returns new ZeroXKeyPasskeyClient
   */
  passkeyClient = (params?: PasskeyClientParams): ZeroXKeyPasskeyClient => {
    const targetRpId =
    ...
    return new ZeroXKeyPasskeyClient({
  ...
  iframeClient = async (
    ...
    const ZeroXKeyIframeElementId =
      params.iframeElementId ?? "zeroxkey-default-iframe-element-id";
    ...
    return new ZeroXKeyIframeClient({
      stamper: this.stamper,
      apiBaseUrl: this.config.apiBaseUrl,
      organizationId: this.config.defaultOrganizationId,
    });
  ...
  walletClient = (wallet: WalletInterface): ZeroXKeyWalletClient => {
    return new ZeroXKeyWalletClient({
  ...
  indexedDbClient = async (): Promise<ZeroXKeyIndexedDbClient> => {
    this.stamper = new IndexedDbStamper();

    return new ZeroXKeyIndexedDbClient({
      stamper: this.stamper,
      apiBaseUrl: this.config.apiBaseUrl,
      organizationId: this.config.defaultOrganizationId,
    });

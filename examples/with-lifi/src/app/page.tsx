...
export default function BridgePage() {
  const { httpClient, session, fetchWalletAccounts, wallets } = useZeroXKey();
  ...
  const [transactionRequest, setTransactionRequest] = useState<
    any | undefined
  >();
  const [zeroXKeySolanaSigner, setZeroXKeySolanaSigner] = useState<
    ZeroXKeySigner | undefined
  >(undefined);
  ...
  useEffect(() => {
    const getWalletAccounts = async () => {
      ...
      try {
        ...
        // create a viem account with the 0xkey wallet
        ...
        setViemAccount(zeroXKeyAccount as Account);
        ...
        // create a ZeroXKey Solana signer
        const zeroXKeySolSigner = new ZeroXKeySigner({
          organizationId: walletAccountResponse[0].organizationId,
          client: httpClient!,
        });

        setZeroXKeySolanaSigner(zeroXKeySolSigner);
  ...
  const handleSwap = async () => {
    ...
    if (fromToken === "ETH") {
      ...
      await zeroXKeySolanaSigner?.addSignature(hexTransaction, solAddress!);

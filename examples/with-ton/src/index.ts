...
async function createWalletTransferV4WithZeroXKey(args: {
  seqno: number;
  sendMode: number;
  walletId: number;
  messages: any[];
  ...
  // Sign message using ZeroXKey
  const txSignResult = await args.zeroXKeyClient.apiClient().signRawPayload({
...
async function main() {
  ...
  const body = await createWalletTransferV4WithZeroXKey({
    seqno,

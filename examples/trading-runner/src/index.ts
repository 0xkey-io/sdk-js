...
async function setup(_options: any) {
  ...
  // setup private keys
  await createPrivateKey(zeroXKeyClient.apiClient(), "Trading Wallet", [
    tradingTagId,
  ]);
  await createPrivateKey(zeroXKeyClient.apiClient(), "Long Term Storage", [
    longTermStorageTagId,
  ]);
  await createPrivateKey(zeroXKeyClient.apiClient(), "Personal", [personal]);
  ...
  const longTermStoragePrivateKey = (
    await getPrivateKeysForTag(zeroXKeyClient.apiClient(), "long-term-storage")
  )[0];
...
async function wrapUnwrapImpl(baseAsset: string, baseAmount: string) {
  ...
  const tradingPrivateKey = (
    await getPrivateKeysForTag(zeroXKeyClient.apiClient(), "trading")
  )[0];

...
async function main() {
  ...
  const condition = `tron.tx.contract[0].contract_address == '${usdtContractAddressNile}' && tron.tx.contract[0].data.startsWith('a9059cbb')`;
  ...
  const newUserZeroXKeyClient = new ZeroXKey({
    apiBaseUrl: "https://api.0xkey.com",
  ...
  const zeroXKeyAddress = process.env.TRON_ADDRESS!;
  ...
  try {
    ...
    await newUserZeroXKeyClient.apiClient().signTransaction({
  ...
  try {
    ...
    const signedTx = await newUserZeroXKeyClient.apiClient().signTransaction({
      signWith: zeroXKeyAddress,

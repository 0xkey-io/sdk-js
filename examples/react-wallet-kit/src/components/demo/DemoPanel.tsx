...
export default function DemoPanel() {
  ...
  function handleError(error: unknown) {
    if (error instanceof ZeroXKeyError) {
      if (error.code === ZeroXKeyErrorCodes.USER_CANCELED) {
        ...
        >
          <div className="flex items-center justify-between">
            <Menu>
              ...
              <MenuItems
                anchor="bottom start"
                transition
                className="origin-top max-h-10 zeroxkey-scrollbar overflow-y-clip flex flex-col gap-3 transition duration-100 ease-out data-closed:scale-95 data-closed:opacity-0 relative z-50 bg-draggable-background-light dark:bg-draggable-background-dark rounded-lg shadow-lg p-2"
              >
                {wallets.length > 0 &&
                  wallets.map((wallet) => {
          ...
          <Button
            onClick={async () => {
              ...
              const messageToSign = "Signing within ZeroXKey Demo.";

...
// Sign with ZeroXKey (single payload, NO_OP)
async function zeroXKeySignDigestHex(digestHex: string): Promise<string> {
  const tk = new ZeroXKeyServerSDK({
  ...
  if (!sr?.r || !sr?.s) {
    throw new Error(
      `ZeroXKey: missing r/s in response: ${JSON.stringify(resp)}`,
    );
  }
...
async function main() {
  ...
  for (let i = 0; i < UTXOS.length; i++) {
    ...
    const derHex = await zeroXKeySignDigestHex(

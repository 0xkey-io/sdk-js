...
import { useZeroXKey, WalletAccount } from "@0xkey-io/react-wallet-kit";
...
export default function JupiterSwapPage() {
  const { wallets, httpClient, handleLogin, logout, session } = useZeroXKey();
  ...
  const zeroXKeySigner = activeWalletAccount
    ? new ZeroXKeySigner({
  ...
  const handleUltraSwap = async () => {
    if (!activeWalletAccount || !zeroXKeySigner) {
    ...
    try {
      ...
      await zeroXKeySigner.addSignature(tx, activeWalletAccount.address);

...
import { ZeroXKeyClient, type ZeroXKeyApiTypes } from "@0xkey-io/http";
import {
...
export function Connect({ organizationId, onAccountConnected }: ConnectProps) {
  const [zeroXKeyClient, setZeroXKeyClient] = useState<ZeroXKeyClient | null>(
    null,
  );

  const [wallets, setWallets] = useState<ZeroXKeyApiTypes["v1Wallet"][]>([]);
  ...
  const [zeroXKeyProvider, setZeroXKeyProvider] =
    useState<ZeroXKeyEIP1193Provider | null>(null);
  ...
  const fetchWallets = async () => {
    if (zeroXKeyClient && organizationId) {
      try {
        const { wallets } = await zeroXKeyClient.getWallets({ organizationId });
        setWallets(wallets);
  ...
  const handleConfirm = async () => {
    if (selectedWalletId && zeroXKeyProvider && selectedAccount) {
      onAccountConnected({
        walletId: selectedWalletId,
        account: selectedAccount,
        provider: zeroXKeyProvider,
      });
    }
  };

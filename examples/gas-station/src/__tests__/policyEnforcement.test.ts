...
describe("Gas Station Policy Enforcement", () => {
  let zeroXKeyClient: ZeroXKeyServerSDK;
  let subOrgId: string;
  ...
  beforeAll(async () => {
    zeroXKeyClient = new ZeroXKeyServerSDK({
    ...
    const result = await zeroXKeyClient.apiClient().createSubOrganization({
    ...
    const walletResult = await zeroXKeyClient.apiClient().createWallet({
    ...
    await zeroXKeyClient.apiClient().createPolicy(eoaPolicy);
    ...
    // Setup EOA client
    const eoaZeroXKeyClient = new ZeroXKeyServerSDK({
      apiBaseUrl: env.BASE_URL,
    ...
    const eoaAccount = await createAccount({
      client: eoaZeroXKeyClient.apiClient(),
      organizationId: subOrgId,
    ...
    const paymasterZeroXKeyClient = new ZeroXKeyServerSDK({
      apiBaseUrl: env.BASE_URL,
    ...
    const paymasterAccount = await createAccount({
      client: paymasterZeroXKeyClient.apiClient(),
      organizationId: subOrgId,
    ...
    // Ensure Gas Station ABI is uploaded to ZeroXKey (enables ABI-based policies)
    ...
    await zeroXKeyClient.apiClient().createPolicy(paymasterPolicy);
  ...
  describe("Layer 1: EOA Intent Signing Policy", () => {
    ...
    it("should allow EOA to sign DAI transfer after adding DAI policy", async () => {
      ...
      await zeroXKeyClient.apiClient().createPolicy(eoaDaiPolicy);
  ...
  describe("Layer 3: Multi-Approval Consensus", () => {
    ...
    let multiApprovalPaymasterZeroXKeyClient: ZeroXKeyServerSDK;
    ...
    beforeAll(async () => {
      ...
      // Create paymaster wallet
      const paymasterWalletResult = await zeroXKeyClient
        .apiClient()
        .createWallet({
      ...
      await zeroXKeyClient.apiClient().createPolicy(multiApprovalPolicy);
      ...
      // Initialize clients
      const multiApprovalEoaZeroXKeyClient = new ZeroXKeyServerSDK({
        apiBaseUrl: env.BASE_URL,
        apiPrivateKey: eoaPrivateKey,
        apiPublicKey: eoaPublicKey,
      ...
      multiApprovalPaymasterZeroXKeyClient = new ZeroXKeyServerSDK({
        apiBaseUrl: env.BASE_URL,
        apiPrivateKey: paymasterPrivateKey,
        apiPublicKey: paymasterPublicKey,
        defaultOrganizationId: multiApprovalSubOrgId,
      ...
      const multiApprovalEoaAccount = await createAccount({
        client: multiApprovalEoaZeroXKeyClient.apiClient(),
        organizationId: multiApprovalSubOrgId,
    ...
    it("should require 2 approvals for signing intent", async () => {
      ...
      try {
        ...
        throw new Error(
          "Expected ZeroXKeyConsensusNeededError but got success",
        );
      } catch (error: any) {
        expect(error.name).toBe("ZeroXKeyConsensusNeededError");
        expect(error.activityStatus).toBe("ACTIVITY_STATUS_CONSENSUS_NEEDED");
        activityId = error.activityId;
      }

      // Get the pending activity
      const activityResult = await zeroXKeyClient.apiClient().getActivity({
        organizationId: multiApprovalSubOrgId,
        activityId,
      });
      ...
      const secondApprovalResult = await multiApprovalPaymasterZeroXKeyClient
        .apiClient()
        .approveActivity({

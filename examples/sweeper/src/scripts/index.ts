...
import { getZeroXKeyClient, pollTransactionStatus } from "../0xkey";
...
export async function main() {
  ...
  const zeroXKey = getZeroXKeyClient();
  ...
  const { useSponsor } = await prompts({
    type: "confirm",
    name: "useSponsor",
    message: "Use ZeroXKey gas sponsorship for sweep transactions?",
    initial: false,
  });
  sponsor = !!useSponsor;
...
async function sweepTokens(
  ...
  for (const token of network.tokens) {
    ...
    const resp = await zeroXKey.apiClient().getNonces({
      organizationId,
    ...
    // Submit transaction via ZeroXKey
    const { sendTransactionStatusId } = await zeroXKey
      .apiClient()
      .ethSendTransaction({
        organizationId,
        from: ownerAddress,

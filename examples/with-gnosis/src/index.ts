...
async function main() {
  if (
    ...
    // You're missing a private key ID. We'll create one for you via calling the ZeroXKey API.
  ...
  const zeroXKeySigner1 = new ZeroXKeySigner({
    client: zeroXKeyClient,
    organizationId: process.env.ORGANIZATION_ID!,
    signWith: process.env.SIGN_WITH_1!,
  });

  const zeroXKeySigner2 = new ZeroXKeySigner({
    client: zeroXKeyClient,
    organizationId: process.env.ORGANIZATION_ID!,
    signWith: process.env.SIGN_WITH_2!,
  });

  const zeroXKeySigner3 = new ZeroXKeySigner({
    client: zeroXKeyClient,
    organizationId: process.env.ORGANIZATION_ID!,
  ...
  const connectedSigner1 = zeroXKeySigner1.connect(provider);
  const connectedSigner2 = zeroXKeySigner2.connect(provider);
  const connectedSigner3 = zeroXKeySigner3.connect(provider);

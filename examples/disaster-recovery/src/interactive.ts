...
async function mainMenu(): Promise<void> {
  ...
  console.log("╔════════════════════════════════════════════════════════════╗");
  console.log(
    "║         ZEROXKEY DISASTER RECOVERY TOOLKIT                  ║",
  );
  console.log("╚════════════════════════════════════════════════════════════╝");
  console.log();
    ...
    const { action } = await prompts({
      ...
      choices: [
        ...
        },
        {
          ...
          description: "Import a BIP-39 seed phrase into ZeroXKey",
          value: "import-wallet",
        },
        {
...
async function importPrivateKey(): Promise<void> {
  ...
  const initResult = await zeroXKeyClient.apiClient().initImportPrivateKey({
    userId,
  });
  ...
  console.log("Step 3: Import encrypted key to ZeroXKey");

  const { confirm } = await prompts({
    type: "confirm",
    name: "confirm",
    message: `Import key "${keyName}" to ZeroXKey?`,
    initial: true,
  });

  if (!confirm) return;

  const importResult = await zeroXKeyClient.apiClient().importPrivateKey({
    userId,
...
async function sweepFunds(): Promise<void> {
  ...
  console.log();
  console.log("Creating ZeroXKey signing account...");
...
async function escrowSetup(): Promise<void> {
  ...
  console.log("1. Create an encryption keypair in ZeroXKey");
  console.log("2. Encrypt your recovery material with ZeroXKey's public key");
  ...
  console.log("not on ZeroXKey. This creates a 2-of-2 security model.");
  console.log();
  ...
  if (existingKeyId) {
    ...
    if (useExisting) {
      ...
      const result = await createNewEncryptionKey(zeroXKeyClient);
  ...
  console.log("- DO NOT store it on ZeroXKey");
...
async function createNewEncryptionKey(
  ...
  console.log();
  console.log("Step 1: Create encryption keypair in ZeroXKey");
...
async function escrowRecovery(): Promise<void> {
  ...
  console.log();
  console.log("This will:");
  console.log("1. Export the encryption key from ZeroXKey");
  ...
  const { confirmExport } = await prompts({
    type: "confirm",
    name: "confirmExport",
    message: `Export encryption key (${keyIdToUse}) from ZeroXKey?`,
    initial: false,
  });
  ...
  console.log("Step 2: Export encryption key from ZeroXKey");

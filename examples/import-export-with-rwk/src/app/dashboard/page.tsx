...
export default function Dashboard() {
  ...
  const { authState, logout, session, wallets } = zeroXKey;
  ...
  useEffect(() => {
    ...
    async function loadPrivateKeys() {
      ...
      setPkError(null);
      try {
        const keys = (await zeroXKey.fetchPrivateKeys?.()) as
          | v1PrivateKey[]
          | undefined;
        if (!cancelled && keys) {
          setPrivateKeys(keys);
  ...
  const onImportPrivateKey = async () => {
    try {
      ...
      const newPrivateKeyId = await zeroXKey.handleImportPrivateKey(params);
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="border-t border-gray-200 pt-4 space-y-3">
                ...
                <p className="mt-1 text-[10px] text-gray-500">
                  ...
                  See{" "}
                  <a
                    href="https://docs.0xkey.com/concepts/wallets#address-formats-and-curves"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="underline"
                  >
                    ZeroXKey Wallets — Address Formats & Curves
                  </a>{" "}
                  for more details.
                </p>
              </div>

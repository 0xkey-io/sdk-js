...
import { getSignatureFromActivity, type TActivity } from "@0xkey-io/http";
...
async function main() {
  ...
  const { destination } = await prompts([
    {
      ...
      initial: ZEROXKEY_EXAMPLE_RECIPIENT,
    },
  ]);
  ...
  try {
    await zeroXKeySigner.addSignature(transaction, solAddress);
  } catch (error: any) {

...
async function main() {
  ...
  const defaultDestination = ZEROXKEY_EXAMPLE_RECIPIENT;
  ...
  try {
    ...
    signature = await handleActivityError(zeroXKeyClient, error).then(
      (activity?: TActivity) => {
        if (!activity) {
          throw error;
        }
  ...
  print("ZeroXKey-powered signature:", `${bs58.encode(signature)}`);
  ...
  const { amount } = await prompts([
    {
      name: "amount",
      type: "text",
      message: `Amount (in Lamports) to send to ${ZEROXKEY_EXAMPLE_RECIPIENT}:`,
      initial: "100",
  ...
  try {
    signedTransaction = (await zeroXKeySigner.signTransaction(
      transaction,
      solAddress,
    )) as Transaction;
  } catch (error: any) {
    await handleActivityError(zeroXKeyClient, error).then(
      (activity?: TActivity) => {
        if (!activity) {
          throw error;
        }

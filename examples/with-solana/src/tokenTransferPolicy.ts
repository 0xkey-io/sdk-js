 ...
 * It also creates SOME of the ZeroXKey setup state required (new ZeroXKey managed Solana wallet (owner of the sending token address) and non-root user
...
async function setup() {
  ...
  const rootUserClient = new ZeroXKey({
  ...
  // Root user ZeroXKey signer
  const rootUserSigner = new ZeroXKeySigner({
    organizationId,
  ...
  console.log(`ZeroXKey Solana wallet address: ${solAddress}`);
...
async function attemptTransferToken() {
  ...
  const nonRootUserClient = new ZeroXKey({
    apiBaseUrl: process.env.BASE_URL!,
  ...
  const nonRootUserSigner = new ZeroXKeySigner({
    organizationId,
    client: nonRootUserClient.apiClient(),
  });

  // Transfer token from primary to example recipient associated token account\
  // This call uses ZeroXKey's sign transaction endpoint which passes the transaction through the policy engine
...
async function createTokenPolicy() {
  ...
  // IMPORTANT STEP
  // Here, given the mint address of the token being transferred, we calculate the Associated Token Address for the RECEIVING address (in this case, example recipient)
  ...
  // Create policy to allow non root user to send a Solana transaction that only conains a SINGLE instruction --> one SPL token transfer to the Token account address for example recipient
  await createPolicy(
    rootUserClient.apiClient(),
    `Let non root user send SPL transfers to the associated token account of EXAMPLE_RECIPIENT: ${ataExampleRecipient.toString()} given the mint address for the token just created`,
    "EFFECT_ALLOW",
    `approvers.any(user, user.id == '${nonRootUserID}')`,
    `solana.tx.instructions.count() == 1 && solana.tx.spl_transfers.any(transfer, transfer.to == '${ataExampleRecipient.toString()}')`,
    "",
  );
}

...
async function main() {
  // Initialize a ZeroXKey client using the parent organization
  ...
  const subOrg = await zeroXKeyClient.createSubOrganization({
  ...
  const { policyId } = await zeroXKeyDelegated.createPolicy({
    policyName,
    condition,
    consensus,
    effect,
  ...
  const RootQuorum = await zeroXKeyDelegated.updateRootQuorum({
    threshold: 1,

...
export async function main() {
  ...
  const { sponsored } = await prompts({
    type: "confirm",
    name: "sponsored",
    message: "Use ZeroXKey gas sponsorship?",
  });
  ...
  if (sponsored) {
    ...
    swapPayload.gasStationNonce = (
      await zeroXKey.apiClient().getNonces({
        organizationId,
        address: wallet,
        caip2: "eip155:8453",
    ...
    const { nonce } = await zeroXKey.apiClient().getNonces({
      organizationId,
      address: wallet,
      caip2: "eip155:8453",
      nonce: true,
  ...
  const { sendTransactionStatusId: swapTx } = await zeroXKey
    .apiClient()

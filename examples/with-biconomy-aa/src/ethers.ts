...
async function main() {
  ...
  // Initialize a ZeroXKey Signer via Ethers v6
  ...
  // Connect a ZeroXKeySigner to a Biconomy Smart Account Client, defaulting to Sepolia
  ...
  const connect = async (
    ...
    try {
      const smartAccount = await createSmartAccountClient({
        signer: zeroXKeySigner as LightSigner,
  ...
  const smartAccount = await connect(zeroXKeySigner);

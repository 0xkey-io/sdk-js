...
import { getProvider, getZeroXKeySigner } from "./provider";
...
// For demonstration purposes, create a globally accessible ZeroXKeyClient
...
async function setup(_options: any) {
  // setup user tags
  const adminTagId = await createUserTag(zeroXKeyClient, "Admin", []);
  const managerTagId = await createUserTag(zeroXKeyClient, "Manager", []);
  const executorTagId = await createUserTag(zeroXKeyClient, "Executor", []);
  ...
  // setup private keys
  await createPrivateKey(zeroXKeyClient, "Distribution", [distributionTagId]);
  await createPrivateKey(zeroXKeyClient, "Long Term Storage", [
    longTermStorageTagId,
  ]);
  await createPrivateKey(zeroXKeyClient, "Short Term Storage 1", [
    shortTermStorageTagId,
  ]);
  await createPrivateKey(zeroXKeyClient, "Short Term Storage 2", [
    shortTermStorageTagId,
  ]);
  await createPrivateKey(zeroXKeyClient, "Short Term Storage 3", [
    shortTermStorageTagId,
  ]);
...
async function fundImpl() {
  ...
  const connectedSigner = getZeroXKeySigner(
    provider,
...
async function sweepImpl() {
  ...
  for (const pk of shortTermStoragePrivateKeys!) {
    const provider = getProvider();
    const connectedSigner = getZeroXKeySigner(provider, pk.privateKeyId);
...
async function pollAndBroadcastImpl() {
  ...
  const activities = await getActivities(zeroXKeyClient, ACTIVITIES_LIMIT);
...
async function approveActivity(options: any) {
  ...
  const activity = await getActivity(zeroXKeyClient, activityId);

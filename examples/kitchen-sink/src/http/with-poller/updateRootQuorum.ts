...
async function main() {
  ...
  const usersResponse = await zeroXKeyClient.getUsers({
    organizationId: process.env.ORGANIZATION_ID!,
  });
  const whoamiResponse = await zeroXKeyClient.getWhoami({
    organizationId: process.env.ORGANIZATION_ID!,
  });
  const orgConfigsResponse = await zeroXKeyClient.getOrganizationConfigs({
    organizationId: process.env.ORGANIZATION_ID!,
  });

  await zeroXKeyClient.updateRootQuorum({
  ...
  const updatedOrgConfigsResponse = await zeroXKeyClient.getOrganizationConfigs(
    {
      organizationId: process.env.ORGANIZATION_ID!,
    },
  );

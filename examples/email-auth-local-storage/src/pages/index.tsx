...
const ZEROXKEY_EMBEDDED_KEY = "@0xkey-io/embedded_key";
const ZEROXKEY_EMBEDDED_KEY_TTL_IN_MILLIS = 1000 * 60 * 60 * 48; // 48 hours in milliseconds;

const ZEROXKEY_CREDENTIAL_BUNDLE = "@0xkey-io/credential_bundle";
const ZEROXKEY_CREDENTIAL_BUNDLE_TTL_IN_MILLIS = 1000 * 60 * 60 * 48; // 48 hours in milliseconds;
...
export default function AuthPage() {
  ...
  const [zeroXKeyPrivateKey, setZeroXKeyPrivateKey] = useState("");
  const [zeroXKeyPublicKey, setZeroXKeyPublicKey] = useState("");
  ...
  const handleGenerateKey = async () => {
    const existingKey = getItemWithExpiry(ZEROXKEY_EMBEDDED_KEY);
    if (existingKey) {
    ...
    try {
      ...
      setItemWithExpiry(
        ZEROXKEY_EMBEDDED_KEY,
        JSON.stringify(key),
        ZEROXKEY_EMBEDDED_KEY_TTL_IN_MILLIS,
      );
  ...
  const injectCredentials = (data: InjectCredentialsFormData) => {
    ...
    let decryptedData;
    try {
      const embeddedKey = getItemWithExpiry(ZEROXKEY_EMBEDDED_KEY);
      ...
      setItemWithExpiry(
        ZEROXKEY_CREDENTIAL_BUNDLE,
        data.authBundle,
        ZEROXKEY_CREDENTIAL_BUNDLE_TTL_IN_MILLIS,
      );
    } catch (e) {
    ...
    // The decrypted credential is an API keypair that can be used to authenticate ZeroXKey requests
    setZeroXKeyPrivateKey(decryptedData);
    setZeroXKeyPublicKey(getPublicKeyFromPrivateKeyHex(decryptedData));
  ...
  const createWallet = async (data: CreateWalletFormData) => {
    const zeroXKeyClient = new ZeroXKey({
      ...
      apiPublicKey: zeroXKeyPublicKey,
      apiPrivateKey: zeroXKeyPrivateKey,
    ...
    const response = await zeroXKeyClient.apiClient().createWallet({
  ...
  return (
    <main className={styles.main}>
      <a href="https://www.0xkey.com" target="_blank" rel="noopener noreferrer">
        <Image
          src="/logo.svg"
          alt="ZeroXKey Logo"
          className={styles.zeroXKeyLogo}
          width={100}
          height={24}
          priority
        />
        ...
        <div>
          ...
          >
            <label className={styles.label}>
              ...
              <input
                defaultValue={getItemWithExpiry(ZEROXKEY_CREDENTIAL_BUNDLE)}
                className={styles.input}
